$.getScript("/socket.io/socket.io.js", function(){
  ...
  window.addBullet = function(otherPlayer){
    if(otherPlayer.signature && otherPlayer.signature != "player2"){
  ...
  (function(){
    ...
    $("#myCanvas").keypress(function(e){
      ...
      if(keycode == 32){
        var player = animateDrawables['player2']
        player.signature = "player2"
        createBullet(player);
  ...
  function init(){
    function findSpawnPoint(){
      for (var i=maze.length-1; i>-1; i--){
        for (var k=maze[0].length-1; k>-1; k--){
          if(maze[i][k] != 5){
            lastGoodX = k * 40;
            lastGoodY = i * 40;
            return;
    ...
    addDrawables('player2_n', lastGoodX, lastGoodY, 24, 24,'player2');
  ...
  function update(){
    ...
    handleUserInput(dt);
    moveBullets(dt);
    checkBorders(animateDrawables['player2']);
    if(checkStaticCollisions(animateDrawables['player2'])){
      resetPlayerPosition();
    }
  ...
  function updateEnemy(otherPlayer){
    if(!animateDrawables['player1']){
      addDrawables(otherPlayer.name, otherPlayer.x, otherPlayer.y, otherPlayer.width, otherPlayer.height, 'player1')
    }else{

      //TODO: Test if I can just set the local object to be what's being passed in. Pass by reference/value is weird in JS.
      var myEnemy = animateDrawables['player1']
  ...
  function resetPlayerPosition(){
    animateDrawables['player2']['x'] = lastGoodX;
    animateDrawables['player2']['y'] = lastGoodY;
  }
